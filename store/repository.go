...
const DBNAME = "jioStore"

// COLLECTION is the name of the collection in DB
const COLLECTION = "store"
const COLLECTION2 = "accoount"

var productId = 10;

// ******************** Account Database  **********************

// GetPasswordByUser returns a password
func (r Repository) GetPasswordByUser(user string) User {
	...
	fmt.Println("user in GetPasswordByUser", user);

	if err := c.FindId(user).One(&result); err != nil {
...
func (r Repository) AddUser(user User) bool {
	...
	session.DB(DBNAME).C(COLLECTION2).Insert(user)
	if err != nil {
		log.Fatal(err)
		return false
	}
	fmt.Println("Added New User Username - ", user.Username)
	return true
}
...
func (r Repository) GetProductsByString(query string) Products {
	...
	if err != nil {
	 	fmt.Println("Failed to a establish connection to Mongo server:", err)
	}
